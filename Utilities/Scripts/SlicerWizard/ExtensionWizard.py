...
class ExtensionWizard(object):
  ...
  def _execute(self, args):
    ...
    candidateBuiltInTemplatePaths = [
        os.path.join(scriptPath, "..", "..", "Templates"), # Run from source directory
        os.path.join(scriptPath, "..", "..", "..", "share", # Run from install
                     "Slicer-%s.%s" % tuple(__version_info__[:2]),
                     "Wizard", "Templates")
        ]
    builtInTemplatePath = None
    for candidate in candidateBuiltInTemplatePaths:
        if os.path.exists(candidate):
            self._templateManager.addPath(candidate)
            builtInTemplatePath = candidate
    ExtensionDescription.DESCRIPTION_FILE_TEMPLATE = os.path.join(builtInTemplatePath, "Extensions", "extension_description.s4ext.in")
