  ...
  vtkMRMLSubjectHierarchyNode::SubjectHierarchyItemID itemID, vtkMRMLSubjectHierarchyNode::SubjectHierarchyItemID parentItemID )
{
  ...
  foreach (currentPlugin, this->m_RegisteredPlugins)
    {
    double currentConfidence = currentPlugin->canReparentNodeInsideSubjectHierarchy(itemID, parentItemID);
    if (currentConfidence > bestConfidence)
      {
      bestConfidence = currentConfidence;
...
//---------------------------------------------------------------------------
qSlicerSubjectHierarchyAbstractPlugin* qSlicerSubjectHierarchyPluginHandler::findOwnerPluginForSubjectHierarchyNode(
  vtkMRMLSubjectHierarchyNode::SubjectHierarchyItemID itemID )
{
  ...
  foreach (currentPlugin, this->m_RegisteredPlugins)
    {
    double currentConfidence = currentPlugin->canOwnSubjectHierarchyNode(itemID);
    if (currentConfidence > bestConfidence)
      {
      bestConfidence = currentConfidence;
...
//---------------------------------------------------------------------------
qSlicerSubjectHierarchyAbstractPlugin* qSlicerSubjectHierarchyPluginHandler::findAndSetOwnerPluginForSubjectHierarchyNode(SubjectHierarchyItemID itemID)
{
  if (!this->m_SubjectHierarchyNode.GetPointer())
    {
    qCritical() << Q_FUNC_INFO << ": Invalid subject hierarchy node";
  ...
  qSlicerSubjectHierarchyAbstractPlugin* ownerPlugin = this->findOwnerPluginForSubjectHierarchyNode(itemID);
  this->m_SubjectHierarchyNode->SetItemOwnerPluginName(ownerPlugin->name().toLatin1().constData());
  return ownerPlugin;
}

//---------------------------------------------------------------------------
qSlicerSubjectHierarchyAbstractPlugin* qSlicerSubjectHierarchyPluginHandler::getOwnerPluginForSubjectHierarchyNode(SubjectHierarchyItemID itemID)
{
  if (!this->m_SubjectHierarchyNode.GetPointer())
    {
    qCritical() << Q_FUNC_INFO << ": Invalid subject hierarchy node";
  ...
  if (ownerPluginName.empty())
    {
    qCritical() << Q_FUNC_INFO << ": Item '" << this->m_SubjectHierarchyNode->GetItemName(itemID) << "' is not owned by any plugin!";
    return NULL;
    }
  ...
  vtkObject* caller, unsigned long event, void* clientData, void* callData )
{
  ...
  if (event == vtkCommand::DeleteEvent)
    ...
    pluginHandler->setSubjectHierarchyNode(
      vtkSlicerSubjectHierarchyModuleLogic::GetSubjectHierarchyNode(pluginHandler->scene) );
    }
}
