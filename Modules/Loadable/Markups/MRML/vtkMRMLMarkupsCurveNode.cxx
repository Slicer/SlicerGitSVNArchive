...
bool vtkMRMLMarkupsCurveNode::ResampleCurveSurface(double controlPointDistance, vtkMRMLModelNode* modelNode, double maximumSearchRadiusTolerance)
{
	...
	vtkPolyData* surfacePolydata = modelNode->GetPolyData();
	if(!surfacePolydata)
	  {
	...
	for (vtkIdType controlPointIndex = 0; controlPointIndex < interpolatedPoints->GetNumberOfPoints(); controlPointIndex++)
	  ...
	  originalControlPoints->GetPoint(segmentStartIndex, segmentStartPoint);				// get nearest control point
	  this->GetClosestPointPositionAlongCurveWorld(interpolatedPoint, projectedPoint);	// get projection of point to curve
