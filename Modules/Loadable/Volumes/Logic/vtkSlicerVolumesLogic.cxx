...
void vtkSlicerVolumesLogic::SetCompareVolumeGeometryEpsilon(double epsilon)
{
  ...
  if (this->CompareVolumeGeometryEpsilon != positiveEpsilon)
    ...
    precision << std::scientific << this->CompareVolumeGeometryEpsilon;
    // this should now hold a string ending in -N where N is the precision
    size_t negativePos = precision.str().find_last_of('-');
    if (negativePos != std::string::npos)
      {
      std::string precisionString;
      precisionString = precision.str().substr(negativePos + 1);
      if (precisionString.size())
        {
        // convert to integer
        this->CompareVolumeGeometryPrecision = std::atoi(precisionString.c_str());
        }
      else
        {
        // default to 6
      ...
      {
      vtkErrorMacro("SetCompareVolumeGeometryEpsilon: failed to parse "
                    << "the scientific notation form of epsilon ("
                    << precision.str().c_str()
                    << ") for a power of 10 to a negative value, "
                    << "defaulting to precision of 6");
      this->CompareVolumeGeometryPrecision = 6;
      }
    this->Modified();
    }
                                             ...
                                             vtkMRMLScalarVolumeNode *volumeNode2)
{
  ...
  if (!volumeNode1 || !volumeNode2)
    ...
    if (!volumeImage1 || !volumeImage2)
      ...
      double minSpacingLog10 = log10(minSpacing);
      double epsilonLog10 = log10(this->CompareVolumeGeometryEpsilon);
      double logDiff = minSpacingLog10 - epsilonLog10;
      vtkDebugMacro("minSpacingLog10  (" << minSpacingLog10 << ") - epsilonLog10 (" << epsilonLog10 << ") = " << logDiff);
