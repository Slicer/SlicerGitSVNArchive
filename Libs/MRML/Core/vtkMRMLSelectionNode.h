...
class VTK_MRML_EXPORT vtkMRMLSelectionNode : public vtkMRMLNode
{
  ...
  /// the ID of a MRMLLayoutNode
  vtkGetStringMacro (ActivePlotLayoutID );
  void SetActivePlotLayoutID(const char* id);
  void SetReferenceActivePlotLayoutID (const char *id) { this->SetActivePlotLayoutID(id); };
