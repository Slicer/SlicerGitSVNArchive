...
def checkVolumeNodeType(volume_type):
...
def PushToSlicer(sitkimage, NodeName, compositeView=0, overwrite=False):
    ...
    if compositeView in compositeViewMap.keys():
        compositeView = compositeViewMap[compositeView]
    if compositeView == 'foreground':
        pass
    elif compositeView == 'background':
        appLogic = slicer.app.applicationLogic()
        selNode = appLogic.GetSelectionNode()
        volumeNode = slicer.util.getNode( newNode.GetName() )
        selNode.SetReferenceActiveVolumeID(volumeNode.GetID())
        appLogic.PropagateVolumeSelection()
    elif compositeView == 'label':
        newNode.SetAttribute('LabelMap', "1")
        compositeNodes = slicer.util.getNodes("vtkMRMLSliceCompositeNode*")
        for node in compositeNodes.values():
            node.SetBackgroundVolumeID(newNode.GetID())
    else:
        raise Exception("Unknown value for compositeView flag!")
###############
